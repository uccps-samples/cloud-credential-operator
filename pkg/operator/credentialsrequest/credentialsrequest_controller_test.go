...
import (
	...
	minteraws "github.com/uccps-samples/cloud-credential-operator/pkg/aws"
	"github.com/uccps-samples/cloud-credential-operator/pkg/aws/actuator"
	mockaws "github.com/uccps-samples/cloud-credential-operator/pkg/aws/mock"
	...
	schemeutils "github.com/uccps-samples/cloud-credential-operator/pkg/util"
)

var c client.Client

const (
	openshiftClusterIDKey = "uccpClusterID"
)
	...
	tests := []struct {
		...
		},
		{
			...
			existing: []runtime.Object{
				...
				testAWSCredsSecret("kube-system", "aws-creds", testRootAWSAccessKeyID, testRootAWSSecretAccessKey),
				testAWSCredsSecret("uccp-cloud-credential-operator", "cloud-credential-operator-iam-ro-creds", testReadAWSAccessKeyID, testReadAWSSecretAccessKey),
				testClusterVersion(),
				testInfrastructure(testInfraName),
			},
			mockRootAWSClient: func(mockCtrl *gomock.Controller) *mockaws.MockClient {
		...
		},
		{
			...
			existing: []runtime.Object{
				...
				createTestNamespace(testSecretNamespace),
				testCredentialsRequest(t),
				testLegacyAWSCredsSecret("uccp-cloud-credential-operator", "cloud-credential-operator-iam-ro-creds", testReadAWSAccessKeyID, testReadAWSSecretAccessKey),
				testAWSCredsSecret(testSecretNamespace, testSecretName, testAWSAccessKeyID, testAWSSecretAccessKey),
				testClusterVersion(),
				testInfrastructure(testInfraName),
			},
		...
		},
		{
			...
			existing: func() []runtime.Object {
				...
				objects = append(objects, testAWSCredsSecret("uccp-cloud-credential-operator", "cloud-credential-operator-iam-ro-creds", testReadAWSAccessKeyID, testReadAWSSecretAccessKey))
...
const (
	...
	testCRName                        = "uccp-component-a"
	testNamespace                     = "uccp-cloud-credential-operator"
