...
import (
	...
	credreqv1 "github.com/uccps-samples/cloud-credential-operator/pkg/apis/cloudcredential/v1"
	"github.com/uccps-samples/cloud-credential-operator/pkg/cmd/provisioning"
	"github.com/uccps-samples/cloud-credential-operator/pkg/gcp"
	"github.com/uccps-samples/cloud-credential-operator/pkg/gcp/actuator"
...
type: Opaque`
	...
	// IAM service account
	generateCredentialsConfigCmd = "gcloud iam workload-identity-pools create-cred-config projects/%s/locations/global/workloadIdentityPools/%s/providers/%s --service-account=<POPULATE_SERVICE_ACCOUNT_EMAIL> --output-file=%s --credential-source-file=/var/run/secrets/uccp/serviceaccount/token --credential-source-type=text"
...
func NewCreateServiceAccountsCmd() *cobra.Command {
	...
	createServiceAccountsCmd.PersistentFlags().StringVar(&CreateServiceAccountsOpts.CredRequestDir, "credentials-requests-dir", "", "Directory containing files of CredentialsRequests to create gcp service accounts for (can be created by running 'oc adm release extract --credentials-requests --cloud=gcp' against an Uccp release image)")
