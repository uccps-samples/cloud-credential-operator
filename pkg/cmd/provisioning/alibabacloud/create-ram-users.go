...
import (
	...
	"github.com/uccps-samples/cloud-credential-operator/pkg/alibabacloud"
...
type: Opaque`
	...
	ccoctlResourcePrefix = "uccp.io/ccoctl"
)
...
func createComponentPolicy(client alibabacloud.Client, policyName string, statements []credreqv1.AlibabaStatementEntry) error {
	...
	if err != nil {
		...
		if ok && aErr.ErrorCode() == errorPolicyNotExists {
			...
			preq.Description = "Created by Uccp ccoctl"
...
func NewCreateRAMUsersCmd() *cobra.Command {
	...
	createRAMUsersCmd.PersistentFlags().StringVar(&CreateRAMUsersOpts.CredRequestDir, "credentials-requests-dir", "", "Directory containing files of CredentialsRequests to create RAM AK for (can be created by running 'oc adm release extract --credentials-requests --cloud=alibabacloud' against an Uccp release image)")
