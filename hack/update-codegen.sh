...
# that deepcopy-gen can operate in
	...
	fake_repopath=$fake_gopath/src/github.com/uccps-samples/cloud-credential-operator
...
if [[ "$(realpath ${valid_gopath}/src/github.com/uccps-samples/cloud-credential-operator)" == "${REPO_FULL_PATH}" ]]; then
...
GOPATH="${temp_gopath}" GOFLAGS="" bash ${CODEGEN_PKG}/generate-groups.sh "deepcopy" \
	github.com/uccps-samples/cloud-credential-operator/pkg/client \
