...
# Design Principles
  ...
  * The controller should be able to run in either a cluster itself, or in a centralized management cluster, most likely along side [Hive](https://github.com/uccps-samples/hive).
...
Source of templates:
  * https://github.com/uccps-samples/installer/blob/master/data/data/manifests/openshift/cloud-creds-secret.yaml.template
...
```
$ kubectl scale -n uccp-cluster-version deployment.v1.apps/cluster-version-operator --replicas=0
$ kubectl scale -n uccp-cloud-credential-operator deployment.v1.apps/cloud-credential-operator --replicas=0
```
...
Cred Minter should now be running in uccp-cloud-credential-operator.
...
CredentialRequests spec consist of:
 ...
 2. providerSpec - Contains the [cloud provider specific credentials specification](https://github.com/uccps-samples/cloud-credential-operator/tree/master/pkg/apis/cloudcredential/v1).
...
metadata:
  name: uccp-image-registry
  namespace: uccp-cloud-credential-operator
spec:
  secretRef:
    name: installer-cloud-credentials
    namespace: uccp-image-registry
  providerSpec:
    apiVersion: cloudcredential.uccp.io/v1
...
# For OpenShift Second Level Operators
 ...
 1. Your CredentialsRequests should be created in the uccp-cloud-credential-operator namespace.
